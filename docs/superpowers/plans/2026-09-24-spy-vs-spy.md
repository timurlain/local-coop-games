...
### Task 18c: Spy sprite overhaul — long noses, real poses, kufřík in hand

User feedback after first look: the spies need the iconic **long pointy noses** of the Mad comic / 1984 game, with nose animations, and the kufřík must be **in the hand**, not at nose height. Also closes the spec §5 gap (distinct walk ×4, search, swing, block frames instead of jitter/rect tricks).

This is pixel art, so the plan gives the constraints and the controller verifies visually in the browser; the implementer draws the rows.

**Files:**
- Modify: `src/games/spy-vs-spy/render/sprite-data.ts`, `src/games/spy-vs-spy/render/sprites.ts`, `src/games/spy-vs-spy/render/spy.ts`, `src/games/spy-vs-spy/render/victory.ts`
- Modify test: `tests/spy-vs-spy/render.test.ts`

**Sprite constraints**
- Size **21 wide × 24 tall**, facing right, palette chars `o` outline, `b` body, `e` eye, `c` club (brown `#8b5a2b`, same in every palette), `.` transparent.
- **Body centre line = column 10** (the image centre) in every frame, so `drawSprite`'s bottom-centre anchor and horizontal flip keep the body in place; the nose and club may extend right, arms/club back may extend left.
- Head: wide-brim hat (brim ~11 px), head in profile, slit eye, and a **long pointed nose ~8 px** sticking forward (right) with a slightly drooping tip — the silhouette must read as "Spy vs Spy" at 1× zoom.
- Frames (`SPY_FRAMES` keys): `stand`, `walk1`–`walk4` (legs cycle; **nose bobs 1 px up on walk2 and walk4**), `search` (bent forward, nose poking forward/down, arms reaching forward), `swing1` (club raised behind/over head), `swing2` (club struck forward), `block` (club held upright in front), `laugh1`/`laugh2` (head tilted back, **nose pointing up**, body shaking 1 px between the two).
- `SPY_HANDS: Record<SpyFrame, readonly [number, number]>` in sprite-data.ts = the pixel (x, y) of the front hand in each frame (image coordinates); for `laugh1/2` it's the raised hand.

**Behaviour**
- `spy.ts`: walking cycles `walk1..4` at ~8 fps; `searching` uses `search`; `swingAnim > 0` uses `swing1` for the first half of `RULES.swingAnim` then `swing2`; `blocking` uses `block`; remove the old jitter and rect-club drawing. Deaths/angel keep using `stand` (and `sooty`/`soaked`/`ghost` palettes).
- **Kufřík in hand:** when `spy.hand?.kind === 'kufrik'`, draw `drawIcon('kufrik')` so its handle sits at the frame's `SPY_HANDS` point (mirror x when flipped). Other hand items are not drawn (they're small; the HUD shows them). This makes who carries the kufřík visible to the opponent — intended.
- `victory.ts`: winner walks with `walk1..4`, then alternates `laugh1`/`laugh2` (~6 fps) with the **kufřík held up at the raised hand** (no longer at nose height); loser stands.

- [ ] **Step 1:** Update `tests/spy-vs-spy/render.test.ts` sprite test: every `SPY_FRAMES` frame is 21×24, rectangular, palette-only; every frame has an entry in `SPY_HANDS` inside the image bounds; the frame set equals the list above. Run → FAIL.
- [ ] **Step 2:** Draw the frames in `sprite-data.ts` (add `c: '#8b5a2b'` to every spy palette), add `SPY_HANDS`. Run the test → PASS.
- [ ] **Step 3:** Update `spy.ts` and `victory.ts` as described; `npx tsc --noEmit`, `npx vitest run`, `npm run build` clean.
- [ ] **Step 4:** Controller verifies visually in the browser (sprite sheet rendered at 6× via dynamic import, plus in-game and victory screenshots) and iterates with the implementer until the noses read right.
- [ ] **Step 5:** Commit `feat(spy): long-nosed spies with real poses; kufřík carried in hand`.
