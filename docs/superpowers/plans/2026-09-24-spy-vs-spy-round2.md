...
- [ ] Implement per spec §5 incl. colour coding, flashing loose secret, remedy→trap hint, toast (1.5 s) under the frame, low-time blink + beep, map view while held (dots only in visited rooms holding a secret/kufřík). Move the always-on mini-map into the Trapulator device and draw **door connectors** for every door of a visited room + the exit plane marker once seen (pure helper `knownDoors(state, spy)` → list of {room, dir}, unit-tested: only doors of visited rooms, both sides of a door reported once, exit included only after its room was visited). The big map uses the same helper.
